...
public class DefaultMobileDeviceManager extends MobileDeviceManager {
	...
	public void submitTask(TaskProperty edgeTask) {
		...
		SimLogger.getInstance().addLog(task.getCloudletId(), //task.getMobileDeviceId()
				//task.getCloudletId(),
				task.getTaskType(),
	...
	private Task createTask(TaskProperty edgeTask){
		...
		// new task using edgeTask and increment counter to create task
		Task task = new Task(edgeTask.mobileDeviceId, ++taskIdCounter,
				// length and pesNUmber required
				edgeTask.length, edgeTask.pesNumber,
				//inputFilesize and outputFileSize required
				edgeTask.inputFileSize, edgeTask.outputFileSize,
				//utilizationModelCPU, utilizationModel required
				utilizationModelCPU, utilizationModel, utilizationModel);

		//set the owner of this task
		//set owner using UserID 
		task.setUserId(this.getId());
		//using getTaskType():
		task.setTaskType(edgeTask.taskType);	//task.setTaskType(edgeTask.getTaskType());
